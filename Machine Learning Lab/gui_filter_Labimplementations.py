...
filter_type_menu=OptionMenu(filter_frame, var_filter, 'Select filter', 'Reg. Least Squared', 'tsvd')
...
def apply_classify():
    if check_v_fixed.get()==1:
        ...
        t_kcv_idx, avg_err_kcv=kcv(kernel_type, s_value, 'tsvd', trange, Xtr, Ytr, k, task, split_type)
        L=trange[t_kcv_idx]
        ...
        c=regularizedKernLSTrain(Xtr, Ytr, kernel_type, s_value, L)
        contour_var1, contour_var2, contour_var3 = separatingFKernRLS(c, Xtr, kernel_type, s_value, Xts)
        ...
        canvas=FigureCanvasTkAgg(main_plot, plot_frame)
        canvas.get_tk_widget().grid(row=1, column=0)
        separatingFKernRLS(c, Xtr, kernel_type, s_value, Xts)
